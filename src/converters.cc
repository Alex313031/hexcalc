...
short              ConvertToShort(const long double in)      { return static_cast<short>(in); }
int                ConvertToInt(const long double in)        { return static_cast<int>(in); }
long               ConvertToLong(const long double in)       { return static_cast<long>(in); }
long long          ConvertToLongLong(const long double in)   { return static_cast<long long>(in); }
unsigned short     ConvertToUshort(const long double in)     { return static_cast<unsigned short>(in); }
unsigned int       ConvertToUint(const long double in)       { return static_cast<unsigned int>(in); }
unsigned long long ConvertToUint64(const long double in)     { return static_cast<unsigned long long>(in); }
float              ConvertToFloat(const long double in)      { return static_cast<float>(in); }
double             ConvertToDouble(const long double in)     { return static_cast<double>(in); }
long double        ConvertToLongDouble(const long double in) { return in; }
unsigned int       ConvertToDword(const long double in)      { return static_cast<unsigned int>(in); }
unsigned long long ConvertToHex64(const long double in)      { return static_cast<unsigned long long>(in); }
...
std::wstring FmtShort(long double r) {
  if (r > static_cast<long double>(SHRT_MAX)) return L"SHRT_MAX";
  if (r < static_cast<long double>(SHRT_MIN)) return L"SHRT_MIN";
  wchar_t b[kBufSize];
  swprintf(b, kBufSize, L"%hd", ConvertToShort(r));
  return b;
}
std::wstring FmtInt(long double r) {
  if (r > static_cast<long double>(INT_MAX)) return L"INT_MAX";
  if (r < static_cast<long double>(INT_MIN)) return L"INT_MIN";
  wchar_t b[kBufSize];
  swprintf(b, kBufSize, L"%d", ConvertToInt(r));
  return b;
}
std::wstring FmtLong(long double r) {
  if (r > static_cast<long double>(LONG_MAX)) return L"LONG_MAX";
  if (r < static_cast<long double>(LONG_MIN)) return L"LONG_MIN";
  wchar_t b[kBufSize];
  swprintf(b, kBufSize, L"%ld", ConvertToLong(r));
  return b;
}
std::wstring FmtLongLong(long double r) {
  if (r > static_cast<long double>(LLONG_MAX)) return L"LLONG_MAX";
  if (r < static_cast<long double>(LLONG_MIN)) return L"LLONG_MIN";
  wchar_t b[kBufSize];
  swprintf(b, kBufSize, L"%lld", ConvertToLongLong(r));
  return b;
}
std::wstring FmtUshort(long double r) {
  if (r > static_cast<long double>(USHRT_MAX)) return L"USHRT_MAX";
  if (r < 0) return L"0";
  wchar_t b[kBufSize];
...
std::wstring FmtUint(long double r) {
  if (r > static_cast<long double>(UINT_MAX)) return L"UINT_MAX";
  if (r < 0) return L"0";
  wchar_t b[kBufSize];
...
std::wstring FmtUint64(long double r) {
  if (r > static_cast<long double>(ULLONG_MAX)) return L"ULLONG_MAX";
  if (r < 0) return L"0";
  wchar_t b[kBufSize];
...
std::wstring FmtFloat(long double r) {
  if (r >  static_cast<long double>(FLT_MAX)) return L"FLT_MAX";
  if (r < -static_cast<long double>(FLT_MAX)) return L"-FLT_MAX";
  wchar_t b[kBufSize];
...
std::wstring FmtDouble(long double r) {
  if (r >  static_cast<long double>(DBL_MAX)) return L"DBL_MAX";
  if (r < -static_cast<long double>(DBL_MAX)) return L"-DBL_MAX";
  wchar_t b[kBufSize];
...
bool ConvertNumbers() {
  ...
  if (!std::isfinite(ref)) {
    ...
    ErrorBox(mainHwnd, L"Invalid Input",
             L"\"" + std::wstring(input) +
                 L"\" is not a finite number (got inf or nan).");
    return false;
  }
